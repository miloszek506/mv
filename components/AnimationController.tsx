...
export function AnimationController() {
  useLayoutEffect(() => {
    ...
    // Delikatne wygładzenie kółka myszy działa tylko na urządzeniach z precyzyjnym
    // wskaźnikiem. Dotyk, klawiatura i prefers-reduced-motion zachowują natywne zachowanie.
    const canUseWheelSmoothing = window.matchMedia("(hover: hover) and (pointer: fine)").matches;
    if (canUseWheelSmoothing) {
      ...
      const syncScrollPosition = () => {
        if (!isSmoothing) targetScroll = window.scrollY;
      };
      ...
      const handleWheel = (event: WheelEvent) => {
        ...
        if (eventTarget?.closest("input, textarea, select, [contenteditable=\"true\"], [role=\"dialog\"]")) return;
        ...
        targetScroll = Math.min(maxScroll, Math.max(0, targetScroll + delta * 0.82));
        isSmoothing = true;
      ...
      window.addEventListener("keydown", stopSmoothing, { passive: true });
      cleanupListeners.push(() => {
        stopSmoothing();
        window.removeEventListener("wheel", handleWheel);
        window.removeEventListener("scroll", syncScrollPosition);
        window.removeEventListener("keydown", stopSmoothing);
      });
    }
